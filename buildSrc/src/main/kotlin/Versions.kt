object Versions {

    // Gradle
    const val androidGradle = "7.2.1"
    const val advancedBuildVersionGradle = "1.7.4"
    const val navigationSafeArgsGradle = "2.3.5"
    const val kotlinSerializationPluginVersion = "1.6.10"
    const val hiltGradle = "2.42"

    // Core
    const val androidCoreVersion = "1.8.0"
    const val appCompactVersion = "1.4.2"
    const val kotlinVersion = "1.7.0"
    const val timberVersion = "5.0.1"
    const val kotlinSerializationVersion = "1.3.3"
    const val gsonVersion = "2.8.5"
    const val kotlinReflectionVersion = "1.7.10"
    const val workRuntimeVersion = "2.7.0"
    const val lifecycleVersion = "2.4.0"
    const val kotlinxDatetime = "0.3.1"

    // UI
    const val androidXConstraintLayoutVersion = "2.1.4"
    const val materialVersion = "1.6.1"
    const val systemUiControllerVersion = "0.24.12-rc"
    const val activityVersion = "1.3.1"
    const val fragmentVersion = "1.3.6"
    const val androidXSupportVersion = "1.0.0"
    const val nav_version = "2.5.0"
    const val carouselRecyclerViewVersion = "1.2.5"
    const val overScrollDecoratorVersion = "1.1.1"
    const val swipeRefreshLayoutVersion = "1.1.0"
    const val blurViewVersion = "4.0.1"
    const val blurryLayoutVersion = "2.0.0"
    const val paletteApiVersion = "1.0.0"
    const val glideVersion = "4.13.2"

    // Coil
    const val coilVersion = "2.1.0"

    // Hilt
    const val hiltVersion = "2.42"
    const val hiltWorkerVersion = "1.0.0"
    const val hiltCompilerVersion = "1.0.0"
    const val daggerHiltCompilerVersion = "2.42"

    // Network
    const val retrofitVersion = "2.9.0"
    const val okHttpLoggingInterceptorVersion = "4.10.0"
    const val gsonConverterVersion = "2.9.0"
    const val kotlinSerializationConverterVersion = "0.8.0"

    const val ktorVersion = "2.0.3"

    // test
    const val junitVersion = "1.1.3"
    const val espressoVersion = "3.4.0"
}
